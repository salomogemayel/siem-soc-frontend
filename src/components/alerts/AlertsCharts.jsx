...
export default function AlertsCharts({
    ...
    const maxAgentValue = Math.max(
        ...topAgentsData.map((item) => item.value),
        1
    );
    ...
    const openAlertInExplorer = (alert) => {
        ...
        if (alert.rule_id) {
            params.set("rule_id", alert.rule_id);
        }

        if (alert.level !== undefined && alert.level !== null) {
            params.set("level", `exact:${alert.level}`);
        }

        if (alert.agent_id && alert.agent_id !== "-") {
            params.set("agent", alert.agent_id);
        }
        ...
        <div className="grid grid-cols-1 gap-[18px] xl:grid-cols-12">
            <div className="rounded-[14px] border border-slate-100 bg-white p-4 shadow-sm xl:col-span-8">
                ...
                <p className="m-0 mt-1 text-sm text-slate-500">
                    Shows how low, medium, and high alerts change over time.
                </p>
            ...
            <div className="rounded-[14px] border border-slate-100 bg-white p-4 shadow-sm xl:col-span-4">
                ...
                <p className="m-0 mt-1 text-sm text-slate-500">
                    Latest alerts that require attention. Click an alert to investigate.
                </p>

                <div className="mt-4 flex flex-col gap-2.5">
                    {recentHighAlerts.length === 0 ? (
                        <p className="m-0 rounded-xl bg-slate-50 p-4 text-sm text-slate-500">
                        ...
                        recentHighAlerts.map((alert, index) => (
                            <button
                                ...
                                onClick={() => openAlertInExplorer(alert)}
                                className="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-100 bg-white p-3 text-left transition hover:bg-slate-50 hover:shadow-sm"
                            >
                                <div className="min-w-0">
                                    <strong className="line-clamp-2 text-sm font-semibold text-slate-900">
                                        {alert.description}
                                    </strong>
                                ...
                                <span
                                    className={`inline-flex min-w-8 shrink-0 justify-center rounded-full px-2.5 py-1 text-xs font-bold ${getLevelClass(alert.level)}`}
                                >
                                    {alert.level}
                                </span>
                            </button>
            ...
            <div className="rounded-[14px] border border-slate-100 bg-white p-4 shadow-sm xl:col-span-8">
                ...
                <p className="m-0 mt-1 text-sm text-slate-500">
                    Rules that appear most often in recent alerts.
                </p>
                            ...
                            topRulesData.map((rule) => (
                                ...
                                >
                                    <td className="border-b border-slate-100 p-3.5">
                                        <button
                                            type="button"
                                            onClick={() => openRuleInExplorer(rule)}
                                    ...
                                    <td className="border-b border-slate-100 p-3.5 font-semibold text-slate-900">
                                        {rule.count}
                                    </td>
                                </tr>
                            ))
            ...
            <div className="rounded-[14px] border border-slate-100 bg-white p-4 shadow-sm xl:col-span-4">
                ...
                <p className="m-0 mt-1 text-sm text-slate-500">
                    Agents with the highest alert activity.
                </p>
                    ...
                    {topAgentsData.length === 0 ? (
                        ...
                        topAgentsData.slice(0, 5).map((agent) => (
                            <button
                                type="button"
                                key={agent.name}
                                onClick={() => openAgentInExplorer(agent)}
                                className="w-full rounded-xl border border-slate-100 bg-white p-3 text-left transition hover:bg-slate-50 hover:shadow-sm"
                            >
                                ...
                                <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                                    <div
                                        ...
                                        style={{
                                            width: `${(agent.value / maxAgentValue) * 100}%`,
                                        }}
                                    />
                                </div>
                            </button>
            ...
            <div className="rounded-[14px] border border-slate-100 bg-white p-4 shadow-sm xl:col-span-8">
                ...
                <p className="m-0 mt-1 text-sm text-slate-500">
                    Alert distribution based on rule group/category.
                </p>
            ...
            <div className="rounded-[14px] border border-slate-100 bg-white p-4 shadow-sm xl:col-span-4">
                ...
                <p className="m-0 mt-1 text-sm text-slate-500">
                    Comparison of alert volume across time.
                </p>
